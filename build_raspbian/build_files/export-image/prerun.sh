...
MINIMUM_IMAGE_SIZE="$((4 * 1024 * 1024 * 1024))"  # was 16GiB.
...
cnt=0
until ensure_next_loopdev && DATA_DEV=$(losetup --show -f -o "${DATA_OFFSET}" --sizelimit "${DATA_LENGTH}" "${IMG_FILE}"); do
	if [ $cnt -lt 5 ]; then
		cnt=$((cnt + 1))
...
mkdosfs -n data -F 32 -v "$DATA_DEV" > /dev/null
...
# I have no idea why pi-gen isn't correctly picking up the custom stage here, but it clearly isn't.
DATA_ROOTFS_DIR="${EXPORT_ROOTFS_DIR/stage2/stageSG}"

rsync -aHAXx --exclude /var/cache/apt/archives --exclude /boot --exclude /data "${EXPORT_ROOTFS_DIR}/" "${ROOTFS_DIR}/"
rsync -rtx "${EXPORT_ROOTFS_DIR}/boot/" "${ROOTFS_DIR}/boot/"
rsync -rtx "${DATA_ROOTFS_DIR}/data/" "${ROOTFS_DIR}/data/"
